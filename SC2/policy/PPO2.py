...
class CriticNet(nn.Module):
    def __init__(self, args, output=1):
        ...
        self.in_to_y1 = nn.Linear(args.state_shape, 100)
...
class Actor:
    def __init__(self, args):
        ...
        self.old_pi = ActorNet(args)
        self.new_pi = ActorNet(args)
        self.optimizer = torch.optim.Adam(self.new_pi.parameters(), lr=args.A_LR, eps=1e-5)
        if args.cuda:
            self.old_pi = self.old_pi.to(args.device)
            self.new_pi = self.new_pi.to(args.device)
            self.old_pi = nn.parallel.DistributedDataParallel(self.old_pi, device_ids=[args.local_rank])
            self.new_pi = nn.parallel.DistributedDataParallel(self.new_pi, device_ids=[args.local_rank])
...
class Critic:
    ...
    def get_v(self, s):
        ...
        if self.args.device != 'cpu':
            inputstate = inputstate.cuda(device=self.args.local_rank)
        return self.critic_v(inputstate)

    def learn(self, reality_v, v):
        loss = self.lossfunc(reality_v, v)
        self.optimizer.zero_grad()
        loss.backward()
...
class PPO:
    ...
    def learn(self, batch, max_episode_len, train_step):
        ...
        for i in range(self.args.batch_size):
            buffer_s = batch['s'][i, :]
            buffer_a = batch['a'][i, :, -1, -1]
            buffer_r = batch['r'][i, :]
            buffer_ap = batch['ap'][i, :, 0]
            buffer_av = batch['avail_a'][i, :, -1, :6]
            buffer_o = batch['o'][i, :, -1, :]
            ...
            bs = torch.FloatTensor(buffer_s)
            ba = torch.FloatTensor(buffer_a)
            br = torch.FloatTensor(discounted_r)
            bap = torch.FloatTensor(buffer_ap)
            bo = torch.FloatTensor(buffer_o)
