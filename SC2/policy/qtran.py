...
class QTran:
    def __init__(self, args, itr):
        ...
        if self.args.alg == 'qtran_base':
            ...
            raise Exception('QTRAN only supports qtran_base!')

        self.v = QtranV(args)

        if self.args.cuda:
            self.eval_rnn, self.target_rnn, self.eval_joint_q, self.target_joint_q =\
                nn.DataParallel(self.eval_rnn), nn.DataParallel(self.target_rnn), nn.DataParallel(self.eval_joint_q),\
                    nn.DataParallel(self.target_joint_q)
            self.eval_rnn.cuda()
