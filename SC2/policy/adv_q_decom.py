...
class Q_Decom:
    def __init__(self, args, itr, load_model=False):
        ...
        if self.load_model:
            if os.path.exists(self.load_dir + '/rnn_net_params.pkl'):
                path_rnn = self.load_dir + '/rnn_net_params.pkl'
                path_mix = self.load_dir + '/' + self.args.alg + '_net_params.pkl'
                ...
                if self.wqmix > 0:
                    path_agent_rnn = self.load_dir + '/rnn_net_params2.pkl'
                    path_qstar = self.load_dir + '/' + 'qstar_net_params.pkl'
                ...
                raise Exception("non exist:" + self.load_dir)
    ...
    def learn(self, batch, max_episode_len, train_step):
        ...
        # print(f"loss: {loss}, eq: {eval_q_total.sum()}, tq: {target_q_total.sum()}, td_e: {td_error.sum()}")
        sys.stdout.flush()
