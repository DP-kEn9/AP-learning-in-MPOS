...
class Runner:
    ...
    def generate_episode(self, episode_num, evaluate=False):
        ...
        for step in range(self.args.episode_limit):
            if done:
                if not evaluate:
                    loss = self.loss_func(reward_all,
                                          torch.FloatTensor(episode_buffer['r'])[:reward_all.shape[0]][:, 0])
                ...
                if not evaluate:
                    ...
                    episode_buffer['a'][step] = np.reshape(actions[self.args.n_agents - self.args.n_adv:], [self.args.n_adv, 1])
