...
class AdvBuffer:
    def __init__(self, args):
        ...
        self.buffer = {
            'o': np.zeros(
                [self.args.buffer_size, self.args.episode_limit, self.args.n_adv, self.args.obs_shape]),
            's': np.zeros(
                [self.args.buffer_size, self.args.episode_limit, self.args.state_shape]),
            'a': np.zeros(
                [self.args.buffer_size, self.args.episode_limit, self.args.n_adv, 1]),
            'onehot_a': np.zeros(
                [self.args.buffer_size, self.args.episode_limit, self.args.n_adv, self.args.n_actions]),
            'avail_a': np.zeros(
                [self.args.buffer_size, self.args.episode_limit, self.args.n_adv, self.args.n_actions]),
            'r': np.zeros(
