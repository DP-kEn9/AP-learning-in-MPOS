...
import numpy as np
from common.replay_buffer import ReplayBuffer as ReplayBuffer
...
class Runner:
    def __init__(self, env, args, itr, seed):
        ...
        self.args = args
        self.arg = copy.deepcopy(args)
        self.arg.n_agents = self.arg.n_agents - self.arg.n_adv

        self.env = env
        self.pid = itr

        self.replay_buffer = ReplayBuffer(self.arg)
        ...
        self.adv_agent = AdvAgents(args, itr=itr, load_model=True)
    ...
    def generate_episode(self, episode_num, evaluate=False, pre_train=False):
        ...
        reward_all = None
        if not evaluate:
            episode_buffer = {'o': np.zeros([self.args.episode_limit, self.args.n_agents - self.args.n_adv, self.args.obs_shape]),
                              's': np.zeros([self.args.episode_limit, self.args.state_shape]),
                              'a': np.zeros([self.args.episode_limit, self.args.n_agents - self.args.n_adv, 1]),
                              'onehot_a': np.zeros([self.args.episode_limit, self.args.n_agents - self.args.n_adv, self.args.n_actions]),
                              'avail_a': np.zeros([self.args.episode_limit, self.args.n_agents - self.args.n_adv, self.args.n_actions]),
                              'r': np.zeros([self.args.episode_limit, 1]),
                              'r_m': np.zeros([self.args.episode_limit, 1]),
                              'next_o': np.zeros([self.args.episode_limit, self.args.n_agents - self.args.n_adv, self.args.obs_shape]),
                              'next_s': np.zeros([self.args.episode_limit, self.args.state_shape]),
                              'next_avail_a': np.zeros(
                                  [self.args.episode_limit, self.args.n_agents - self.args.n_adv, self.args.n_actions]),
        ...
        for step in range(self.args.episode_limit):
            if done:
                ...
                if not evaluate:
                    ...
                    episode_buffer['a'][step] = np.reshape(actions[:self.args.n_agents - self.args.n_adv], [self.args.n_agents - self.args.n_adv, 1])
