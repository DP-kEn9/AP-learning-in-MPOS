...
def plot(mapname, different, reward):
    ...
    for i in range(1, len(itr_0)):
        if 5 * i >= min_len:
            ...
            break
        wr_0.append(win_rates_0[5 * (i - 1):5 * i])
        wr_1.append(win_rates_1[5 * (i - 1):5 * i])
        writr_0.append(sum(itr_0[5 * (i - 1):5 * i]) / 5)
        writr_1.append(sum(itr_1[5 * (i - 1):5 * i]) / 5)
    ...
    if "8m" not in mapname or different == "reward":
        ...
        sns.lineplot(x="T (mil)", y="win_rates", data=plot_wr2, ax=ax1, ci='sd', estimator=np.average, color="green").set_title("6h_vs_8z")
        if different != "reward":
...
def plot_8m_reward(j):
    ...
    fig.savefig("C:\\Users\\48326\\Desktop\\experiment\\experiment data\\8m_reward_.png")
    return fig
...
def plot_one(path, j=5, fig=None, color="green", map=""):
    ...
    for i in range(1, len(itr)):
        if j * i >= len(itr):
            ...
            break
        wr.append(win_rates[j * (i - 1):j * i])
        writr.append(sum(itr[j * (i - 1):j * i]) / j)
    wr, writr = np.array(wr), np.array(writr)
    wr = wr.flatten()[:, None]
...
if __name__ == '__main__':
    ...
    fig = plot_one(path, color="blue", map="1c_vs_30zg")
    fig.savefig("C:\\Users\\48326\\Desktop\\experiment\\experiment data\\1c_vs_30zg.png")
