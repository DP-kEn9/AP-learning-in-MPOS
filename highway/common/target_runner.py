...
class Runner:
    def __init__(self, env, args, itr, seed):
        ...
        self.crashed = False
        self.speed = []
        self.total_speeds = np.zeros(self.args.control_units + self.args.other_vehicle)
        ...
        self.victim = None
        if self.args.training == "adv":
            self.victim = Agents(args, vic=True)
                                 
                                                                     
        self.reward_net = Reward_net(args.state_shape, args.rnn_hidden_dim)
    ...
    def generate_episode(self, episode_num, evaluate=False, pre_train=False):
        ...
        last_action = np.zeros((self.args.control_units, self.args.n_actions))
        ...
        obs = []
        if self.args.obs_type == "TimeToCollision":
            for i in range(self.args.control_units):
                obs.append(obs_[0].flatten())
        else:
            for i in obs_:
                obs.append(i.flatten())
        obs = np.array(obs)
        state = obs[self.args.agent_pos].flatten()
                                                     
                                 
                         
        self.agents.policy.init_hidden(1)
        if self.args.training == "adv":
            self.victim.policy.init_hidden(1)
                                              
        avail_actions = []
        a_actions = list(self.env.env.env.get_available_actions())
        for a in a_actions:
            aa = np.zeros(self.args.n_actions)
            aa[a] = 1
        ...
        for step in range(self.args.episode_limit):
            if done:
                ...
                for agent_id in range(self.args.control_units):
                                                                                                           
                                                                                                       
                    if agent_id not in self.args.agent_pos:
                        if self.args.training != "adv":
                            action = np.random.choice(np.nonzero(avail_actions[agent_id])[0])
                        else:
                            action = self.victim.choose_action(obs[agent_id], last_action[agent_id], agent_id,
                                                               avail_actions[agent_id], epsilon, True)[0]
                ...
                obs_, reward, done, truncated, info = self.env.step(actions_)
                obs = []
                if self.args.obs_type == "TimeToCollision":
                    for i in range(self.args.control_units):
                        obs.append(obs_[0].flatten())
                ...
                vic_crashed = 0
                if self.args.training == "adv":
                    for v in self.env.env.env.controlled_vehicles[:-self.args.n_agents]:
                        vic_speed += v.speed
                        target_speed = v.target_speed
                        if v.crashed:
                            vic_crashed += 1
                    self.speed.append(vic_speed / (self.args.control_units - self.args.n_agents))
                else:
                    for v in self.env.env.env.controlled_vehicles:
                        vic_speed += v.speed
                        target_speed = v.target_speed
                        if v.crashed:
                            vic_crashed += 1
                    self.speed.append(vic_speed / self.args.n_agents)
                if self.args.training == "adv" and (self.time_steps >= 100000 or pre_train):
                    if self.args.reward_model == 0:
                        reward = self.get_adv_reward(vic_crashed=vic_crashed, vic_speed=vic_speed)
                    else:
                        reward_m, self.hidden = self.reward_net(torch.FloatTensor(state), self.hidden)
                ...
                next_avail_actions = []
                na_actions = list(self.env.env.env.get_available_actions())
                for a in na_actions:
                    aa = np.zeros(self.args.n_actions)
                    aa[a] = 1
                    next_avail_actions.append(aa)
                actions, onehot_actions, avail_actions, next_avail_actions_ = np.array(actions), np.array(
                    onehot_actions), np.array(avail_actions), np.array(next_avail_actions)
                      
                if not evaluate:
                    episode_buffer['o'][step] = obs[self.args.agent_pos]
                    episode_buffer['s'][step] = state
                                                           
                                                         
                                              
                    episode_buffer['a'][step] = np.reshape(actions[self.args.agent_pos], [self.args.n_agents, 1])
                    episode_buffer['onehot_a'][step] = onehot_actions[self.args.agent_pos]
                    episode_buffer['avail_a'][step] = avail_actions[self.args.agent_pos]
                    episode_buffer['r'][step] = [reward]
                    episode_buffer['next_o'][step] = next_obs[self.args.agent_pos]
                    episode_buffer['next_s'][step] = next_state
                    episode_buffer['next_avail_a'][step] = next_avail_actions_[self.args.agent_pos]
        ...
        if done:
            if self.args.map == "highway-v0":
                win = self.crashed or np.average(np.array(self.speed)) <= 20
                if self.args.training == "adv":
                    for v in self.env.env.env.controlled_vehicles[:-self.args.n_agents]:
                        if v.crashed or np.average(np.array(self.speed)) <= 20:
                            win = True
                            value_true = 10
                        else:
                            win = False
                ...
                if self.args.training == "adv":
                    for v in self.env.env.env.controlled_vehicles[:-self.args.n_agents]:
                        if v.crashed:
                            ...
                            win = False
                            value_true = -10
            if self.args.training == "adv" and (self.time_steps >= 100000 or pre_train) and self.args.reward_model == 1:
                if abs(reward_all.sum() - value_true) < 2 and reward_all.sum()*value_true > 0:
                    self.store = True
                else:
                    self.store = False
                                                                             
                loss = self.loss_func(reward_all.sum(), torch.FloatTensor([value_true])[0])
    ...
    def run(self):
        ...
        while self.time_steps < self.args.max_steps:
            ...
            if self.replay_buffer.size < self.args.batch_size * self.args.bs_rate:
                print('Replay buffer has not reached batch size * {} yet.'.format(self.args.bs_rate))
                begin_time = None
                begin_step = None
                continue
            ...
            if (self.time_steps - last_test_step) / self.args.evaluation_steps_period >= 1.0:
                ...
                if num_eval % 50 == 0:
                    ...
                    print("Estimated remaining time: {}".format(str(expected_remain_time)))
                    print("Estimated end time: {}".format(expected_end_time.strftime("%Y-%m-%d_%H-%M-%S")))
    ...
    def evaluate(self):
        ...
        for itr in range(self.args.evaluate_num):
            ...
            av_speed = np.average(np.array(self.speed))
            episodes_speed.append(av_speed)
            if win:
                win_number += 1
        if self.args.training == "adv":
            wr = win_number / self.args.evaluate_num
        else:
            wr = 1 - win_number / self.args.evaluate_num
        print("speed:" + str(np.average(np.array(episodes_speed))))
        return wr, episodes_reward
    ...
    def get_adv_reward(self, vic_crashed=0, vic_speed=0, target_speed=25):
        ...
        if self.args.map == "highway-v0":
            reward = -10 * adv_crashed + 10 * (self.args.n_agents + 1) * vic_crashed +\
                     (vic_speed / (self.args.control_units - self.args.n_agents) - target_speed)
        else:
            reward = -10 * adv_crashed + 10 * (self.args.n_agents + 1) * vic_crashed
    ...
    def pre_train_reward_model(self):
        ...
        for itr in range(1000):
            ...
            loss_all += loss
            if loss < 2 and reward_all * value_true > 0:
                T += 1
            else:
                F += 1
            if itr%10 == 0:
                acc = T / (T + F)
                mean_loss = loss_all / 10
                loss_all = 0
                T = 0
                F = 0
                print("acc:" + str(acc) + "   loss:" + str(mean_loss))
