...
class advQMIXMixer(nn.Module):
    def __init__(self, args):
        super(advQMIXMixer, self).__init__()
        self.args = args
        if args.two_hyper_layers:
            self.hyper_w1 = nn.Sequential(nn.Linear(args.state_shape, args.hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(args.hyper_hidden_dim, args.n_adv * args.qmix_hidden_dim))
            ...
            self.hyper_w1 = nn.Linear(args.state_shape, args.n_adv * args.qmix_hidden_dim)
    ...
    def forward(self, qsas, states):
        ...
        qsas = qsas.view(-1, 1, self.args.n_adv)
        states = states.reshape(-1, self.args.state_shape)

        w1 = torch.abs(self.hyper_w1(states))
        w1 = w1.view(-1, self.args.n_adv, self.args.qmix_hidden_dim)
