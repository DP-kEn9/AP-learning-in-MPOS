...
class Q_Decom:
    def __init__(self, args, itr):
        ...
        if args.load_model:
            if os.path.exists(self.args.load_dir + '/rnn_net_params.pkl'):
                ...
                print('loading model %s'%path_rnn + ' 和 %s'%path_mix)
            else:
                raise Exception("model unexist")
        ...
        print("alg " + self.args.alg + " initialized")
    ...
    def learn(self, batch, max_episode_len, train_step):
        ...
        weights = torch.as_tensor(np.ones(eval_q_total.shape), dtype=torch.float)
        if self.wqmix > 0:
